...
public class Lozinki {
	public static void main (String[] args) throws IOException {
		...
		CBHT<String,String> tbl = new CBHT<String,String>(N);
		String vlez="";
		for(int i=1;i<=N;i++){
			String imelozinka = br.readLine();
			String[] pom = imelozinka.split(" ");
			tbl.insert(new String(pom[0]), new String(pom[1]));
		}
		while(!((vlez = br.readLine()).equals("KRAJ"))) {
			String[] pom = vlez.split(" ");
			if(tbl.search(pom[0])==null || !((tbl.search(pom[0])).getElement().value.equals(pom[1]))){
