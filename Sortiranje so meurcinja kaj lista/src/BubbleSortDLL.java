...
public class BubbleSortDLL {
	...
	private static void bubbleSort(DLL<Integer> lista) {
		
		for(int i=0;i<lista.length()-1;i++) {
			DLLNode<Integer> tmp = lista.getFirst();
			while(tmp!=null) {
				DLLNode<Integer> pom  = tmp.succ;
				if(tmp.element > pom.element) {
					lista.insertBefore(pom.element, tmp);
					lista.insertBefore(tmp.element, pom);
					lista.delete(pom);
					tmp=tmp.pred;
